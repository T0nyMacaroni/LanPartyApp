...
public class Reader {
	 ...
	 * 
	 * @param fileName
	 *            - String :: has to be an absolute path
	 * @return Object
	 */
	...
	private static void prepare_player_data(String path) {
		if (path == null)
			path = Settings.PLAYER_REPOSITORY_DATA;
		List<Player> repo_player = (List<Player>) readFile(path);
		if (repo_player == null) return;
	...
	private static void prepare_games_data() {
		...
		if (repo_games == null) return;
		GamesRepository.createMap(repo_games);
	}

	/**
	 * Imports player repository data from a file to an object using an absolute
	 * path
	 * 
	 * @param filePath
	 *            - absolute path
